...
int main() {
    ...
    SDL_Window* window = SDL_CreateWindow("Ready for member 2!", 
        SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 900, 700, SDL_WINDOW_SHOWN);
    SDL_Renderer* renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED);
    
    TTF_Font* font = TTF_OpenFont("arial.ttf", 28);
    
    std::string textBuffer = "Write text! Ready for member 2! (triggers)";
    SDL_StartTextInput();
    
    bool running = true;
    Uint32 lastTriggerCheck = 0;  // Для таймера триггера
    ...
    while (running) {
                    ...
                    case SDLK_F2:
                        {
                            ...
                            f << textBuffer;
                            std::cout << "SAVED: " << textBuffer.length() << " chars" << std::endl;
                        }
                        break;
                    case SDLK_F3:
                        {
                            ...
                            if (f) {
                                ...
                                f.seekg(0);
                                f.read(textBuffer.data(), textBuffer.size());
                            }
        ...
        // Готово для триггеров: textBuffer доступен
        ...
        std::string instr = "ESC=exit F2=save F3=load | Backspace | Enter=test | Длина: " + std::to_string(textBuffer.length());
